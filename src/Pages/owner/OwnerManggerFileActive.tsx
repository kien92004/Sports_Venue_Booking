    ...
    const fetchFieldData = async (date: string) => {
        try {
            ...
            const response = await fetch(
                `${URL_BACKEND}/api/field-usage/active-fields/list-fields?date=${date}&ownerUsername=${ownerUsername}`
            );
            if (response.ok) {
        ...
        return (
            <div className={`card h-100 shadow-sm border-0 position-relative ${hasBookings ? '' : 'bg-light'}`}
                ...
                <div className="card-body">
                    <NavLink to={`/owner/manager-file-active-detail/${field.fieldId}`} className="text-decoration-none">
